...
<div class="container">
    ...
    <table>
        ...
        <tr><td>WS_des</td><td>Gió KCN & Trụ (V_tk={m.VB}m/s)</td><td>Tổ hợp Cường độ III (không có xe)</td><td>H_ngang={loads.loads_stem_base.get('WS_ngang_des', loads.loads_stem_base.get('WS_ngang', None)).Hy if loads.loads_stem_base.get('WS_ngang_des') else 0.0:.1f} kN, H_dọc={loads.loads_stem_base.get('WS_doc_des', loads.loads_stem_base.get('WS_doc', None)).Hx if loads.loads_stem_base.get('WS_doc_des') else 0.0:.1f} kN</td><td>Đỉnh gối & thân</td></tr>
        <tr><td>WS_25</td><td>Gió KCN & Trụ (V=25m/s)</td><td>Tổ hợp Cường độ V & Sử dụng I (có xe)</td><td>H_ngang={loads.loads_stem_base.get('WS_ngang_25').Hy if loads.loads_stem_base.get('WS_ngang_25') else 0.0:.1f} kN, H_dọc={loads.loads_stem_base.get('WS_doc_25').Hx if loads.loads_stem_base.get('WS_doc_25') else 0.0:.1f} kN</td><td>Đỉnh gối & thân</td></tr>
        <tr><td>WL</td><td>Gió thổi lên hoạt tải xe (0.14 kN/m²)</td><td>Tổ hợp Cường độ V & Sử dụng I</td><td>H_ngang={loads.loads_stem_base.get('WL_ngang').Hy if loads.loads_stem_base.get('WL_ngang') else 0.0:.1f} kN, H_dọc={loads.loads_stem_base.get('WL_doc').Hx if loads.loads_stem_base.get('WL_doc') else 0.0:.1f} kN</td><td>Cao 1.8m trên mặt cầu</td></tr>
        <tr><td>WA & WB</td><td>Dòng chảy nước & Đẩy nổi</td><td>p = 0.514·CD·V² (V={m.V_water}m/s)</td><td>WA={loads.WA_total:.1f} kN, WB={loads.WB_total:.1f} kN</td><td>Đáy thân & bệ</td></tr>
        <tr><td>TU+ / TU-</td><td>Biến dạng nhiệt độ (ΔT=±{m.delta_T_pos:.0f}°C)</td><td>Giải chuỗi gối 1D</td><td>TU+ = {getattr(loads.bearing_forces, 'H_TU_pos', 0.0):.1f} kN, TU- = {getattr(loads.bearing_forces, 'H_TU_neg', 0.0):.1f} kN</td><td>Mặt gối đỉnh trụ</td></tr>
